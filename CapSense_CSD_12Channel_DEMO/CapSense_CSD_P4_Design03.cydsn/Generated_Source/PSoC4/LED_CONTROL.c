...
void LED_CONTROL_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        LED_CONTROL_CONTROL_REG = LED_CONTROL_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        LED_CONTROL_TRIG_CONTROL1_REG  = LED_CONTROL_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        LED_CONTROL_SetInterruptMode(LED_CONTROL_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        LED_CONTROL_SetCounterMode(LED_CONTROL_COUNT_DOWN);
        LED_CONTROL_WritePeriod(LED_CONTROL_QUAD_PERIOD_INIT_VALUE);
        LED_CONTROL_WriteCounter(LED_CONTROL_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (LED_CONTROL__QUAD == LED_CONTROL_CONFIG) */

    #if (LED_CONTROL__TIMER == LED_CONTROL_CONFIG)
        LED_CONTROL_CONTROL_REG = LED_CONTROL_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        LED_CONTROL_TRIG_CONTROL1_REG  = LED_CONTROL_TIMER_SIGNALS_MODES;
        ...
        #if (LED_CONTROL_CY_TCPWM_V2 && LED_CONTROL_TIMER_UPDOWN_CNT_USED && !LED_CONTROL_CY_TCPWM_4000)
            LED_CONTROL_WriteCounter(1u);
        #elif(LED_CONTROL__COUNT_DOWN == LED_CONTROL_TC_COUNTER_MODE)
            LED_CONTROL_WriteCounter(LED_CONTROL_TC_PERIOD_VALUE);
        #else
            LED_CONTROL_WriteCounter(0u);
        #endif /* (LED_CONTROL_CY_TCPWM_V2 && LED_CONTROL_TIMER_UPDOWN_CNT_USED && !LED_CONTROL_CY_TCPWM_4000) */
    #endif  /* (LED_CONTROL__TIMER == LED_CONTROL_CONFIG) */

    #if (LED_CONTROL__PWM_SEL == LED_CONTROL_CONFIG)
        LED_CONTROL_CONTROL_REG = LED_CONTROL_CTRL_PWM_BASE_CONFIG;

        #if (LED_CONTROL__PWM_PR == LED_CONTROL_PWM_MODE)
            LED_CONTROL_CONTROL_REG |= LED_CONTROL_CTRL_PWM_RUN_MODE;
            LED_CONTROL_WriteCounter(LED_CONTROL_PWM_PR_INIT_VALUE);
        #else
            LED_CONTROL_CONTROL_REG |= LED_CONTROL_CTRL_PWM_ALIGN | LED_CONTROL_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (LED_CONTROL_CY_TCPWM_V2 && LED_CONTROL_PWM_UPDOWN_CNT_USED && !LED_CONTROL_CY_TCPWM_4000)
                LED_CONTROL_WriteCounter(1u);
            #elif (LED_CONTROL__RIGHT == LED_CONTROL_PWM_ALIGN)
                LED_CONTROL_WriteCounter(LED_CONTROL_PWM_PERIOD_VALUE);
            #else 
                LED_CONTROL_WriteCounter(0u);
            #endif  /* (LED_CONTROL_CY_TCPWM_V2 && LED_CONTROL_PWM_UPDOWN_CNT_USED && !LED_CONTROL_CY_TCPWM_4000) */
        #endif  /* (LED_CONTROL__PWM_PR == LED_CONTROL_PWM_MODE) */

        #if (LED_CONTROL__PWM_DT == LED_CONTROL_PWM_MODE)
            LED_CONTROL_CONTROL_REG |= LED_CONTROL_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (LED_CONTROL__PWM_DT == LED_CONTROL_PWM_MODE) */

        #if (LED_CONTROL__PWM == LED_CONTROL_PWM_MODE)
            LED_CONTROL_CONTROL_REG |= LED_CONTROL_CTRL_PWM_PRESCALER;
        #endif  /* (LED_CONTROL__PWM == LED_CONTROL_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        LED_CONTROL_TRIG_CONTROL1_REG  = LED_CONTROL_PWM_SIGNALS_MODES;
...
void LED_CONTROL_WriteCompare(uint32 compare)
{
        ...
        if (((uint32)LED_CONTROL__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)LED_CONTROL__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 LED_CONTROL_ReadCompare(void)
{
        ...
        regVal = LED_CONTROL_COMP_CAP_REG;
        
        if (((uint32)LED_CONTROL__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)LED_CONTROL__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & LED_CONTROL_16BIT_MASK);
    #else
...
void LED_CONTROL_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)LED_CONTROL__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)LED_CONTROL__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
uint32 LED_CONTROL_ReadCompareBuf(void)
{
        ...
        regVal = LED_CONTROL_COMP_CAP_BUF_REG;
